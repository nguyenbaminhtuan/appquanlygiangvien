<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Chỉnh sửa Học vị/Học hàm cho: {{ $lecturer->full_name }}
        </h2>
    </x-slot>
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
            ...
            @endif
            <form method="POST" action="{{ route('admin.lecturers.academic-degrees.update', ['lecturer' => $lecturer->id, 'degree' => $degree->id]) }}">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Dropdown chọn Loại Bằng cấp --}}
                    <div>
                        ...
                        <select id="degree_type_id" name="degree_type_id" required
                            ...
                            @foreach ($degreeTypes as $type)
                                <option value="{{ $type->id }}" {{ old('degree_type_id', $degree->degree_type_id) == $type->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input id="specialization" type="text" name="specialization" value="{{ old('specialization', $degree->specialization) }}" required class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
                    </div>

                    <div>
                        <label for="issuing_institution" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Nơi cấp</label>
                        <input id="issuing_institution" type="text" name="issuing_institution" value="{{ old('issuing_institution', $degree->issuing_institution) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
                    </div>
                    <div>
                        <label for="date_issued" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Ngày cấp</label>
                        <input id="date_issued" type="date" name="date_issued" value="{{ old('date_issued', $degree->date_issued ? \Carbon\Carbon::parse($degree->date_issued)->format('Y-m-d') : '') }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">
                    </div>
                    <div class="md:col-span-2">
                        <label for="notes" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Ghi chú</label>
                        <textarea id="notes" name="notes" rows="3" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">{{ old('notes', $degree->notes) }}</textarea>
                    </div>
                </div>
